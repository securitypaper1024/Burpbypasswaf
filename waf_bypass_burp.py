# -*- coding: utf-8 -*-
"""
WAF Bypass Encoder - Burp Suite Python Extension
通过各种编码方式绕过WAF检测

安装方法:
1. Burp Suite -> Extender -> Options -> Python Environment
2. 设置Jython standalone JAR路径
3. Extender -> Extensions -> Add -> Extension Type: Python
4. 选择此文件
...
class BurpExtender(IBurpExtender, ITab, IContextMenuFactory):
    ...
    def registerExtenderCallbacks(self, callbacks):
        ...
        self._stdout.println("Author: Security Researcher")
        self._stdout.println("Encodings: IBM037, IBM500, IBM1026, UTF-16, UTF-32, ISO-8859, Windows-1252")
        self._stdout.println("=" * 50)
...
class WAFBypassPanel(JPanel):
            ...
            def run(self):
                for i, (encoding, enc_type) in enumerate(ENCODINGS, 1):
                    try:
                        ...
                        # 更新表格 - 使用工厂函数捕获当前循环变量的值
                        # 避免Python闭包问题（所有闭包共享同一个变量引用）
    ...
    def createContextMenu(self, invocation):
        ...
        # 发送完整请求到面板（仅查看，不自动编码）
        send_item = JMenuItem("Send Full Request to Panel (View Only)")
        ...
        class SendToPanel(ActionListener):
            ...
            def actionPerformed(self, event):
                ...
                if request:
                    ...
                    self._panel._log("[*] Tip: Click 'Parse & Extract' to extract body, click 'Fuzz All' to encode")
